...
- 本番LayerでのDI解決
- テストLayerへの差し替え
- ネストProvider（子が親のLayerに追加のLayerをmerge）
- Layerのリソースクリーンアップ

**参考**: ユーザーの既存パターン（`effect-app-demo`での`Context.Tag` + `Layer.succeed` + `Layer.scoped`）

### 3.2 キャッシュ・無効化

- [ ] キーベースのキャッシュ管理
- [ ] `invalidateQueries(keyPattern)`（TanStack Query風）
- [ ] staleTime / gcTime 設定
- [ ] キャッシュの手動クリア
- [ ] ウィンドウフォーカス時の自動再フェッチ（オプション）

**テスト計画**:
- staleTime内は再フェッチしない
- staleTime後は再フェッチ
- invalidate → アクティブなObserverへの再フェッチ
- gcTime後の非アクティブエントリ削除
- キーパターンマッチング
