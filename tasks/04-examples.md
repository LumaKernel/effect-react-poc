...
- [ ] EffectProvider + useEffectQuery の最小例
- [ ] Effect.succeed → 即座に値を表示
- [ ] Layer不要の最もシンプルな形
...
- [ ] SubscriptionRefを使ったカウンター
- [ ] increment/decrement ボタン
- [ ] 複数コンポーネントでの共有状態
- [ ] リアルタイム更新
...
- [ ] Todo追加・削除・更新・一覧
- [ ] Schema.TaggedClassでTodo型定義
- [ ] Layer経由のStorageService（localStorage）
- [ ] テスト用のInMemoryStorageService
...
- [ ] @effect/platform のHttpClientを使用
- [ ] ローディング状態 → データ表示
- [ ] Suspense版とnon-Suspense版の比較
- [ ] エラー時のErrorBoundary表示
...
- [ ] Stream.paginateChunkEffect でストリーミング取得
- [ ] IntersectionObserver でスクロール検知
- [ ] useEffectStream での逐次表示
